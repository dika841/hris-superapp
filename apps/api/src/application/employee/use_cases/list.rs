...
impl<R: EmployeeRepository> ListEmployeesUseCase<R> {
    ...
    pub async fn execute(&self, page: u64, page_size: u64, department: Option<&str>) -> Result<(Vec<Employee>, u64), RepositoryError> {
        self.employee_repository.list(page, page_size, department).await
    }
}
